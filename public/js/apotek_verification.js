        ...
        $(`#${status}_apotek_verification_table`).DataTable({
            ...
            columns: [
                ...
                {data: 'id', render: function(data, type, row, meta) {
                    ...
                    html += '<button class="btn_detail btn btn-inline-block btn-secondary mr-1" type="button" data-toggle="modal" onclick="detailModal()" data-id="'+data+'" data-name="'+row.user.name+'" data-email="'+row.user.email+'" data-phone="'+row.user.phone+'" data-photo="'+row.user.photo+'" data-name_apotek="'+row.name+'" data-address="'+row.address+'" data-latitude="'+row.latitude+'" data-longitude="'+row.longitude+'" data-ktp="'+row.ktp+'" data-npwp="'+row.npwp+'" data-surat_izin_usaha="'+row.surat_izin_usaha+'" data-image_apotek="'+row.image+'" data-gender="'+gender+'">Detail</button>'
